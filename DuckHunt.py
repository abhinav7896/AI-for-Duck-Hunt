...
class Cursor(games.Sprite):
    ...
    def tick(self):
        ...
        if not Game.paused and not Game.over:
            ...
            if (games.mouse.is_pressed(0) or games.screen.ai_fire()) and not Cursor.clicked:
                # Play Gunshot Sound and add Total Sounds
                print(games.screen.ai_fire())
                Cursor.clicked = True
            ...
            if Cursor.clicked:
                if self.mouseCounter > 3 and not (games.mouse.is_pressed(0) or games.screen.ai_fire()):
...
class Duck(games.Sprite):
    ...
    def __init__(self, duckType):     
        ...
        super(Duck, self).__init__(image=self.flyRight[1], x=randint(10, 470), y=350, dx=0, dy=-1*duckType)
        
        # Point Values Based On Duck Color
        pointValues = {"black": 25, "blue": 50, "red": 75}
    ...
    def tick(self):
        ...
        if not Game.paused:
            if not self.alive:
                ...
                if self.dieDelay > 10 and not self.continueDeath:
                    self.dy = 3.5
...
class Game(games.Sprite):
    ...
    def spawn(self):
        ...
        batch_size = np.random.randint(4, 8)
...
# Syntax.......: main()
# ==========================================
def main(use_AI=False, agent=None):
    # Setup background
    ...
    games.screen.mainloop(use_AI, agent)
...
path1 = './PERCEPTION/raw/*'
path2 = './PERCEPTION/detection/*'
clear_folder(path1)
clear_folder(path2)

# Start!
main(use_AI=True, agent=agent)
